...
class Listen(interactions.Extension):
    ...
    async def on_message_reaction_add(self, reaction: interactions.MessageReaction):
        logging.info(f"{reaction.user_id}")
        logging.info(f"{reaction.channel_id}")
        logging.info(f"{reaction.message_id}")
        logging.info(f"{reaction.emoji}")
        logging.info(f"{reaction.member}")
    ...
    # Reaction Roles
    @interactions.extension_listener()
    async def on_raw_reaction_remove(self, reaction: interactions.MessageReaction):
        guild_id = str(reaction.guild_id)
        message_id = str(reaction.message_id)
        emoji = str(reaction.emoji)
